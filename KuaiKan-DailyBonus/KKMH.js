    ...
    $.get({
      ...
      try {
        ...
        if (cc.code == 200) {
          ...
          $.msg($.name, rep ? '今天已签过！' : pop, `${rep?``:text+', '}现有${$.score||score}积分, ${$.kkb||kkb}KK币\n${gift||title} 🎉`, imgUrl)
    ...
    $.get({
      ...
      try {
        ...
        if (cc.code == 200 && cc.data) {
          ...
          if (cc.data.giftBagSupplement) $.gifts += `, 补签胶囊`;
          if (cc.data.giftBagCardCoupon) $.gifts += `, ${cc.data.cardCoupon.title}`;
          if (cc.data.giftBagYouzanCoupon) $.gifts += `, ${cc.data.cardCoupon.title}`;
        } else {
          $.log(`${$.name} 失败${Details}`)
