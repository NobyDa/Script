...
async function SwitchRegion(play) {
	const Group = $.read('BiliArea_Policy') || 'Bili_AUTO_REGIONS'; //Your blibli policy group name.
	const CN = $.read('BiliArea_CN') || 'DIRECT'; //Your China sub-policy name.
	const TW = $.read('BiliArea_TW') || 'A_TW'; //Your Taiwan sub-policy name.
	const HK = $.read('BiliArea_HK') || 'A_HK'; //Your HongKong sub-policy name.
	const DF = $.read('BiliArea_DF') || 'DF'; //Sub-policy name used after region is blocked(e.g. url 404)
	const off = $.read('BiliArea_disabled') || 'A_TW'; //WiFi blacklist(disable region change), separated by commas.
