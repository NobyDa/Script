...
async function all() {//签到模块相互独立,您可注释某一行以禁用某个接口.
    ...
    await Promise.all([JingDongBean(stop), JingRongBean(stop), JingRongSteel(stop), JingDongTurn(stop), JRDoubleSign(stop), JDGroceryStore(stop), JingDongClocks(stop), JingDongPet(stop), JDFlashSale(stop), JingDongBook(stop), JDSecondhand(stop), JingDMakeup(stop), JingDongWomen(stop), JingDongCash(stop), JingDongShoes(stop), JingRongGame(stop), JingDongLive(stop), JingDongClean(stop), JDPersonalCare(stop), JingDongPrize(stop), JingDongShake(stop)])

    await Promise.all([TotalSteel(), TotalCash(), TotalBean()])
